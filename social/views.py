...
class UserSearch(View):
    def get(self, request, *args, **kwargs):
        query = request.GET.get('q')  # Obtener el término de búsqueda desde el input
        
        if query:
            # Buscar usuarios cuyo username o email coincida con la búsqueda
            users = User.objects.filter(
                Q(username__icontains=query) | Q(email__icontains=query)
            )
        else:
            users = User.objects.none()  # No se devuelve nada si no hay término de búsqueda
...
class UserProfileView(View):
    def get(self, request, username, *args, **kwargs):
        ...
        return render(request, 'pages/social/profile.html', context) 